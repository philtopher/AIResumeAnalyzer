...
// Update the sendEmail function with better logging and error handling
async function sendEmail({
  ...
  try {
    ...
    console.log('[SendGrid] Queueing email:', {
      to,
      from,
      replyTo,
      subject
    ...
    const msg = {
      ...
      attachments,
      headers: {
        'X-Entity-Ref-ID': `${Date.now()}-${Math.random()}`, // Prevent caching
      }
    };

    return new Promise((resolve, reject) => {
      emailQueue.push({ params: msg, resolve, reject });
      processEmailQueue();
    });
  } catch (error: any) {
    console.error('[SendGrid] Email queueing error:', {
      error: error.message,
      stack: error.stack
    });
    return false;
